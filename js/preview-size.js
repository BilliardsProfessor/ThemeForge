(function () {
    ...
    const previewResizeThreshold =
        parseWidthValue(rootStyles.getPropertyValue("--preview-resize-threshold").trim()) || 3;
    ...
    function showCustomToolbar(showToolbar) {
        ...
        previewCustomToolbar.classList.toggle("is-visible", showToolbar);
        previewCustomToolbar.setAttribute("aria-hidden", String(!showToolbar));

        if (previewCustomWidthInput) {
            previewCustomWidthInput.disabled = !showToolbar;
        }

        if (previewCustomResetBtn) {
            previewCustomResetBtn.disabled = !showToolbar;
        }
    }
    ...
    function setPreviewSize(sizeName) {
        const nextSizeName =
            sizeName === "custom" || Object.prototype.hasOwnProperty.call(previewSizes, sizeName)
                ? sizeName
                : "desktop";
